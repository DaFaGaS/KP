...
class TechMenu(Frame):
    def create_widgets(self, controller):
        ...
        self.upper_info = tk.CTkLabel(self, text=f'Главное меню Специалиста:', font=("Verdana", 20),fg_color="#65ab9e")
        ...
        self.request_taken = tk.CTkButton(self, width=200, height=64, bg_color="black", text="Взятые\nзаявки",command=lambda: controller.show_taken_requests())
        self.request_taken.place(anchor='w', relx=0, y=32)
        ...
        self.updatebutton=tk.CTkButton(self,width=200, height=64,text="Обновить", bg_color="black", command=lambda : controller.update_tech())
        self.updatebutton.place(anchor='w', relx=0.689, y=32)

        self.updatebutton = tk.CTkButton(self, width=200, height=64, text="Список\nзаявок", bg_color="black", command=lambda: controller.show_requests_list())
        ...
        self.requests_list = ScrollFrame(self)
        #       ----------USERFRAME----------

        #       ----------REQUEST_FRAMES----------
        self.req1_label = tk.CTkLabel(self, text=f'Заявка №1', font=("Verdana", 16), fg_color="#86e3d2")
        self.req1_label.place(x=225, rely=0.12)
        self.req1_button = tk.CTkButton(self, width=300, text="Информация", bg_color="#86e3d2", command=lambda: controller.see_content_tech(1))
        ...
        self.req1_change = tk.CTkButton(self, width=300, text="Сменить статус",state="disabled", bg_color="#86e3d2", fg_color="#088a58", command=lambda: controller.change_status_window(0))
        self.req1_change.place(x=125, rely=0.27)

        self.req2_label = tk.CTkLabel(self, text=f'Заявка №2', font=("Verdana", 16), fg_color="#86e3d2")
        self.req2_label.place(x=225, rely=0.37)
        self.req2_button = tk.CTkButton(self, width=300, text="Информация", bg_color="#86e3d2", command=lambda: controller.see_content_tech(2))
        ...
        self.req2_change = tk.CTkButton(self, width=300, text="Сменить статус",state="disabled", bg_color="#86e3d2", fg_color="#088a58", command=lambda: controller.change_status_window(1))
        self.req2_change.place(x=125, rely=0.52)

        self.req3_label = tk.CTkLabel(self, text=f'Заявка №3', font=("Verdana", 16), fg_color="#86e3d2")
        self.req3_label.place(x=225, rely=0.62)
        self.req3_button = tk.CTkButton(self, width=300, text="Информация", bg_color="#86e3d2", command=lambda: controller.see_content_tech(3))
        ...
        self.req3_change = tk.CTkButton(self, width=300, text="Сменить статус",state="disabled", bg_color="#86e3d2", fg_color="#088a58", command=lambda: controller.change_status_window(2))
        self.req3_change.place(x=125, rely=0.77)
        #       ----------REQUEST_FRAMES----------
